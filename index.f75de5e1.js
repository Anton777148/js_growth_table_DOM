...
document.addEventListener("DOMContentLoaded", ()=>{
    ...
    const updateButtonStates = ()=>{
        ...
        let columnCount = 0;
        if (table.rows.length > 0 && table.rows[0].cells.length > 0) columnCount = table.rows[0].cells.length;
        else columnCount = 0;
    ...
    const addColumn = ()=>{
        ...
        if (rows[0].cells.length < maxColumns) rows.forEach((row)=>{
            row.insertCell();
        });
        updateButtonStates();
    };
    const remoweColumn = ()=>{
        const rows = document.querySelectorAll("table tr");
        if (rows[0].cells.length > 2) rows.forEach((row)=>{
            row.deleteCell(-1);
        });
        updateButtonStates();
    };
